...
test_fm_home_parameterization() {
  ...
  grep -F "'$ROOT/bin/fm-status-append.sh' '$home_one/state/task-a.status'" "$brief" >/dev/null || fail "brief did not route the FM_HOME state path through the status appender"
  ...
  grep -F "'$ROOT/bin/fm-status-append.sh' '$home_one/state/task-b.status'" "$brief" >/dev/null || fail "scout brief did not route the FM_HOME state path through the status appender"
  ...
  grep -F "'$ROOT/bin/fm-status-append.sh' '$home_one/state/task-c.status'" "$brief" >/dev/null || fail "secondmate brief did not route the FM_HOME state path through the status appender"
