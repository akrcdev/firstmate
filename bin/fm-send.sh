...
fm_send_close_resolved_keys() {  # <answer-text>
  ...
  for k in $RESOLVE_STATUS_KEYS; do
    ...
    if [ "$append_rc" -eq 2 ]; then
      echo "error: the answer was delivered to $T, but decision key '$k' could not be closed in $RESOLVE_STATUS_FILE. Close it manually with: $SCRIPT_DIR/fm-status-append.sh '$RESOLVE_STATUS_FILE' 'resolved [key=$k]: <how it was answered>' - do not resend the answer." >&2
      return 1
    fi
  done
}
