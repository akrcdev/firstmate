...
All current lifecycle producers serialize appends with decision presentation through `bin/fm-status-append.sh` or the equivalent `fm-wake-lib.sh` primitive.
Generated briefs seal that writer contract in `fm-brief-provenance.v1`: explicitly tagged task and charter bodies may be filled before first delivery, while all other generated instructions must still match.
The first verified launch snapshot pins the complete bytes that later relaunches must match unless an owner-mediated instruction update, such as the control relaunch progress-note transaction, regenerates the provenance.
Spawn delivers those verified bytes from a read-only `state/<id>.brief.snapshot` and records `status_writer_protocol=fm-status-append.v1` only from that verified brief identity, so adding a protocol marker to legacy instructions cannot promote their writer.
The [`afk` skill](../.agents/skills/afk/SKILL.md) owns the resulting away-mode compatibility gates for existing tasks and new launches.
