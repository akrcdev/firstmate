...
`data/projects.md` records each project's standing posture and optional `+yolo` merge flag as the captain's default and as context for that decision, including the conditional `no-mistakes-prod-only` policy; `AGENTS.md` section 7 owns that policy's selective per-task classification, compatibility, and unchanged authority boundaries.
A ship spawn that differs from a registered flat posture prints a deviation notice and continues; the notice distinguishes a reasoned increase in rigor from a reduction that requires a current explicit captain instruction.
