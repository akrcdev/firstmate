...
All current lifecycle producers serialize appends with decision presentation through `bin/fm-status-append.sh` or the equivalent `fm-wake-lib.sh` primitive, and new task metadata records the `fm-status-append.v1` writer protocol.
Away-mode entry rejects an unsupported local protocol and a live local pre-version writer because either could append outside that lock; retained stopped pre-version tasks and remote secondmates are outside that live-local compatibility gate.
