...
A healthy normal Pi extension-owned replacement arm emits `check: rearm-resurface` only when the durable queue has a row to present.
On that path, an empty recovery episode is retired under the queue and recovery locks without creating a user message or model turn, so a racing append is either presented by that cycle or opens a fresh generation for the next poll.
Abnormal recovery and non-Pi arm paths remain presentable.
...
An episode that presented durable work is retired by the generation-bound acknowledgement the drain prints as `WAKE_ACK_REQUIRED`.
On a healthy normal Pi extension-owned replacement, an episode with no durable queue row is retired internally before any notification because there is no handling turn to acknowledge it.
Other and abnormal recovery paths retain presentation and generation-bound acknowledgement.
An unacknowledged downtime generation is announced at most once: the first recovery with durable work marks that generation announced, and later arms wait until a new down stretch mints a new generation.
A non-successor watcher start after an announced-but-unacked episode is a new down stretch and mints a fresh generation, then re-presents recovery unless the healthy normal Pi replacement policy can retire an empty episode quietly.
...
`tests/fm-watch-arm.test.sh` covers durable queue replay, healthy Pi-owned quiet empty recovery followed by a real wake, real remote parent-replies ingestion into the authoritative status log, interrupted handling replay, generation-bound acknowledgement, a persistent live successor after recovery, a watcher close inside the handling window that must leave the printed acknowledgement valid, and the self-healing moved-generation acknowledgement that consumes its handled rows and names its remedy.
`tests/fm-watch-recovery-loop.test.sh` drives the real Pi extension and arm/watcher interface across an empty session replacement, proves the next real wake is delivered exactly once, covers the once-per-generation announcement bound against a refused handling handshake, and proves a handling successor surfaces a real crew event instead of going blind.
