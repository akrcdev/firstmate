...
- `no-mistakes-prod-only` is the conditional validation-path policy whose complete per-task classification is owned by `AGENTS.md` section 7.
...
Existing registry entries follow the compatibility rule in `AGENTS.md` section 7, while a legacy entry with no bracket continues to parse as `no-mistakes`.
Registering a conditional policy is a one-time choice and never requires classifying any change; the per-task classification happens at each task's intake under `AGENTS.md` section 7.
...
A `no-mistakes` or `no-mistakes-prod-only` project must have an `origin` remote and must complete the initialization procedure below, because the conditional policy can resolve tasks to the pipeline.
